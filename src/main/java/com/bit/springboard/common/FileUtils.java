...
public class FileUtils {
	...
	public BoardFile parseFileInfo(MultipartFile file, String directoryPath) throws IOException {
		String bucketName = "bitcamp-bucket-136";
		...
		//try 구문안에서만 사용할 객체나 변수를 선언할 수 있음
		//주로 사용후에 close 해줘야하는 객체들을 선언한다.
		try(InputStream fileInputStream = file.getInputStream()) {
			ObjectMetadata objectMetadata = new ObjectMetadata();
			objectMetadata.setContentType(file.getContentType());

			PutObjectRequest putObjectRequest = new PutObjectRequest(
					bucketName, directoryPath + boardFileName, fileInputStream,
					objectMetadata
